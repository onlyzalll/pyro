    ...
    async def export_folder_link(self: "pyrogram.Client", folder_id: int) -> str:
        ...
        r = await self.invoke(
            raw.functions.chatlists.ExportChatlistInvite(
                ...
                title=folder.title,
                peers=(
                    [await self.resolve_peer(i.id) for i in folder.pinned_peers]
                    if folder.pinned_peers
                    else (
                        []
                        + [await self.resolve_peer(i.id) for i in folder.included_peers]
                        if folder.included_peers
                        else (
                            []
                            + [
                                await self.resolve_peer(i.id)
                                for i in folder.excluded_peers
                            ]
                            if folder.excluded_peers
                            else []
                        )
                    )
                ),
